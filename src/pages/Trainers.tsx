...
import { Users, UserPlus, DollarSign, Plus, Dumbbell, Loader2, Pencil, Trash2, CheckCircle, Copy } from "lucide-react";
...
const Trainers = () => {
  ...
  const [form, setForm] = useState({ full_name: "", phone: "", email: "", password: "", specialization: "", salary: "", status: "active" });
  ...
  const resetForm = () => { setForm({ full_name: "", phone: "", email: "", password: "", specialization: "", salary: "", status: "active" }); setNextCode(""); setEditItem(null); };

  const openAdd = async () => { resetForm(); setNextCode(await generateCode()); setOpen(true); };

  const openEdit = (t: Trainer) => {
    setEditItem(t);
    setForm({ full_name: t.full_name, phone: t.phone || "", email: "", password: "", specialization: t.specialization || "", salary: t.salary?.toString() || "", status: t.status });
    setOpen(true);
  };

  const handleSave = async () => {
    if (!form.full_name) { toast.error("Name required"); return; }
    if (!editItem && !form.email) { toast.error("Email required for new trainers"); return; }
    if (!editItem && !form.password) { toast.error("Password required"); return; }
      ...
      if (editItem) {
        ...
        const res = await supabase.functions.invoke("create-user", {
          body: {
            ...
            full_name: form.full_name, role: "trainer", gym_id: gym.id,
          },
        });
        ...
        const { error } = await supabase.from("trainers").insert({
          ...
          specialization: form.specialization || null, salary: form.salary ? parseFloat(form.salary) : null,
          gym_id: gym.id, user_id: userId || null,
        });
        if (error) throw error;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <button onClick={openAdd} className="px-4 py-2 bg-accent text-accent-foreground rounded-lg text-sm font-medium hover:bg-accent/90 transition-colors flex items-center gap-2 self-start"><Plus className="w-4 h-4" /> Add Trainer</button>
      </div>
          ...
          <div className="space-y-3 py-2">
            ...
            {!editItem && (
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-label mb-1 block">Email *</label><input type="email" className="w-full bg-input rounded-lg px-3 py-2 text-sm text-foreground" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="coach@email.com" /></div>
                <div><label className="text-label mb-1 block">Password *</label><input type="password" className="w-full bg-input rounded-lg px-3 py-2 text-sm text-foreground" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="••••••••" /></div>
              </div>
            )}
            <div className="grid grid-cols-2 gap-3">
      ...
      <Dialog open={!!credentialsModal} onOpenChange={() => setCredentialsModal(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><CheckCircle className="w-5 h-5 text-primary" /> Trainer Created!</DialogTitle>
            <DialogDescription>Share these credentials with the trainer.</DialogDescription>
          </DialogHeader>
          {credentialsModal && (
            <div className="space-y-3 py-2">
              <div className="bg-surface rounded-lg p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <div><p className="text-label">Trainer Code</p><p className="text-sm text-foreground font-mono font-semibold">{credentialsModal.code}</p></div>
              ...
              <p className="text-xs text-muted-foreground text-center">⚠️ Save these — the password won't be shown again.</p>
            </div>
          )}
          <DialogFooter>
      ...
      <Dialog open={!!deleteItem} onOpenChange={(v) => !v && setDeleteItem(null)}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            <button onClick={() => setDeleteItem(null)} className="px-4 py-2 text-sm text-muted-foreground hover:text-foreground">Cancel</button>
            <button onClick={handleDelete} disabled={saving} className="px-4 py-2 bg-destructive text-destructive-foreground rounded-lg text-sm font-medium hover:bg-destructive/90 disabled:opacity-50 flex items-center gap-2">{saving && <Loader2 className="w-4 h-4 animate-spin" />} Delete</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
